...
export const QuickStoryDialog = ({ open, onOpenChange, influencerId, influencerName, onSaved }: Props) => {
  ...
  const submit = async () => {
    ...
    const payload = {
      ...
      campaign_name: `Story ${format(date, "dd.MM.yyyy")}`,
      platform: "Story",
      ...
      <DialogContent className="border-border bg-card sm:max-w-md">
        <DialogHeader>
          <DialogTitle className="flex items-center gap-2">
            <Eye className="h-5 w-5 text-[hsl(var(--platform-story))]" /> Log Story
          </DialogTitle>
          <DialogDescription>Quick log an Instagram Story for {influencerName}.</DialogDescription>
        </DialogHeader>
        ...
        <div className="grid gap-4 py-2">
          ...
          <div className="space-y-1.5">
            ...
            <Textarea
              ...
              placeholder="e.g. product showcase, competition announcement…"
              rows={3}
            />
          </div>
        </div>
